...
class MultiStandardDeck extends Deck {
  constructor(options = {}) {
    ...
    for (let i = 0; i < numOfDecks; i++) {this._decks.push(new StandardDeck(options));}
  ...
  //Returns nothing.
  shuffle () {this._decks.forEach(deck => deck.shuffle()) return super.shuffle();}
  ...
  removeCard (card) {this._decks.forEach(deck => deck.removeCard(card));
    return super.removeCard(card);}

  //Add card from the drawn card pile back into the deck.
  //Returns boolean value for success.
  addCard (card) {this._decks.forEach(deck => deck.addCard(card));
